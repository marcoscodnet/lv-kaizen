        ...
        <div class="card-header">
            ...
            <div class="row flex-between-center">
                <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                    <h5 class="fs-9 mb-0 text-nowrap py-2 py-xl-0"><i class="fa fa-truck" aria-hidden="true"></i><span class="ms-2">Proveedores</span></h5>
                </div>
                <div class="col-8 col-sm-auto text-end ps-2">

                    <div id="table-customers-replace-element">
                        <a class="btn btn-falcon-default btn-sm d-inline-flex align-items-center" href="{{ route('proveedors.create') }}">
    ...
    <script>
        $(document).ready(function() {
            $('#example1').DataTable({
                ...
                paging : true,
                "ajax": {
                    "url": "{{ route('proveedors.dataTable') }}",
                    "type": "POST",
                    "data": function (d) {
                    ...
                    // Actions column
                    {
                        ...
                        "render": function(data, type, row) {
                            ...
                            @can('proveedor-ver')
                                ...
                                actionsHtml += '<a href="{{ route("proveedors.show", ":id") }}" class="btn btn-link p-0" alt="Ver" title="Ver" data-bs-toggle="tooltip" data-bs-placement="top"><span class="text-500 fas fa-search"></span></a>'.replace(':id', row.id);
                            @endcan

                            // Agregar enlace de edición si el usuario tiene permiso
                            @can('proveedor-editar')
                                actionsHtml += '<a href="{{ route("proveedors.edit", ":id") }}" class="btn btn-link p-0" alt="Editar" title="Editar" data-bs-toggle="tooltip" data-bs-placement="top"><span class="text-500 fas fa-edit"></span></a>'.replace(':id', row.id);
                            @endcan


                            // Agregar formulario de eliminación si el proveedor tiene permiso
                            @can('proveedor-eliminar')
                                actionsHtml += '<form id="delete-form-' + row.id + '" method="post" action="{{ route('proveedors.destroy', '') }}/' + row.id + '" style="display: none">';
        ...
        function exportarExcel() {
            ...
            let url = "{{ route('proveedors.exportarXLS') }}"
        ...
        function exportarPDF() {
            ...
            let url = "{{ route('proveedors.exportarPDF') }}"
                + "?search=" + encodeURIComponent(busqueda); // <-- pasar búsqueda
